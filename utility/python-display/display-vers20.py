...
class Display:
    ...
    def _ensure_ring(self, r: int, c: int, fg_out: str, fg_in: str, 
        ...
        # Ignore size_px parameter - always use fixed 180px size
        size_px = 180
    ...
    def set_ring_value(self, r: int, c: int, outer: int, inner: int) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Create with defaults - using fixed size 180px
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 180, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
    ...
    def set_ring_text(self, r: int, c: int, text: Optional[str]) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Create ring with defaults - using fixed size 180px
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 180, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
    ...
    def set_ring_extra_arcs(self, r: int, c: int, val1: int, val2: int) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Create ring with defaults if it doesn't exist - using fixed size 180px
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 180, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
