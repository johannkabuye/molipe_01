...
class Display:
    ...
    def _build_ui(self) -> None:
        ...
        # Simulate 5" Raspberry Pi screen (800x480)
        if is_linux():
    ...
    def _ensure_ring(self, r: int, c: int, fg_out: str, fg_in: str, 
        ...
        # Canvas display size is 280px, but arcs are drawn at 180px
        display_size = 280
        ...
        if holder is None:
            holder = tk.Frame(self.cell_frames[r][c], bg="black", bd=0, highlightthickness=0)
            # Place lower (rely=0.6) so arcs have more space above, less below
            holder.place(relx=0.5, rely=0.6, anchor="center", width=display_size, height=display_size)
    ...
    def set_ring_value(self, r: int, c: int, outer: int, inner: int) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Canvas is 280px, arcs drawn at 180px
