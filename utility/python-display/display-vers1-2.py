...
HEAD_ROW_BONUS_PT = -8  # -8 pt for the first row (row 0) as requested
...
class Display:
    ...
    def _build_ui(self):
        # geometry hints aren't needed in fullscreen, but harmless if kept
        if is_linux():
            self.root.geometry("1280x720+0+0")  # typical for the 5" RPi display
        else:
        ...
        for r in range(self.rows):
            ...
            for c in range(cols):
                ...
                if r == 0:
                    fnt = self.head_font               # small + bonus
        ...
        # Remove the interactive fullscreen toggle; keep Escape to quit
        # self.root.bind("<F11>", lambda e: self.root.attributes(
        #     "-fullscreen", not self.root.attributes("-fullscreen")))
        self.root.bind("<Escape>", lambda e: self.root.destroy())

        # bring to front once (useful when launched from a desktop session)
        self.root.lift()
...
def main():
    root = tk.Tk()
    root.title("")                         # no title bar text
    root.overrideredirect(True)            # remove window manager borders/chrome
    root.attributes("-fullscreen", True)   # occupy the entire display
    root.config(cursor="none")             # hide mouse pointer (kiosk feel)
