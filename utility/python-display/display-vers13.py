...
# Rows that contain bar widgets (0-based: row 4 = index 3, row 8 = index 7)
BAR_ROWS = {3, 7}  # Rows 4 and 8
...
class DualRing(tk.Frame):
    ...
    def set_extra_arcs(self, val1: int, val2: int) -> None:
        ...
        # Swap values so val1 controls the closer arc, val2 controls outermost
        self._extra_arc1_val = self._clip_value(val2)
        self._extra_arc2_val = self._clip_value(val1)
        self._update_extents()
    ...
    def _redraw(self) -> None:
        ...
        # Calculate padding - work from inside out
        # Inner arc is the thickest and innermost
        inner_pad = max(self._w_inner // 2 + 2, 4)
        
        # Outer arc (grey, thinner) - outside the inner arc
        outer_pad = inner_pad + self._w_inner // 2 + self._w_outer // 2 + RING_EXTRA_ARC_GAP
        
        # Extra arc 1 (first OUTSIDE outer arc)
        gap = RING_EXTRA_ARC_GAP
        extra_arc_width = RING_EXTRA_ARC_WIDTH
        extra_arc1_pad = outer_pad + self._w_outer // 2 + gap + extra_arc_width // 2
        
        # Extra arc 2 (outermost)
        extra_arc2_pad = extra_arc1_pad + gap + extra_arc_width
        ...
        # Extra arc 1 (light colored, outside grey arc)
    ...
    def _update_extents(self) -> None:
        ...
        s = min(w, h)
        radius1 = s // 2 - extra_arc1_pad
        radius2 = s // 2 - extra_arc2_pad
...
class Display:
    ...
    def set_ring_value(self, r: int, c: int, outer: int, inner: int) -> None:
        ...
        if ring is None:
            ...
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 200, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
    ...
    def set_ring_extra_arcs(self, r: int, c: int, val1: int, val2: int) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Create ring with defaults if it doesn't exist
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 200, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
