...
BIG_FONT_ROWS = {1, 2, 6, 9}

# Rows that contain bar widgets (0-based: row 4 = index 3, row 7 = index 6)
BAR_ROWS = {3, 6}  # Rows 4 and 7
...
BAR_BORDER_COLOR = "#4a4a4a"
BAR_FILL_COLOR = "#4a4a4a"
BAR_BG_COLOR = "#000000"
BAR_GAP_PIXELS = 2  # Gap between border and fill
BAR_BORDER_WIDTH = 2
BARS_PER_CELL = 2  # Two bars stacked vertically per cell
...
def start_udp_listener(out_queue: Queue, metrics: PerformanceMetrics) -> None:
    ...
    def parse_message(line: str) -> Optional[Tuple]:
        ...
        try:
            # BAR r c bar_index value
            if head == "BAR" and len(parts) >= 5:
                r, c = int(parts[1]), int(parts[2])
                bar_index, value = int(parts[3]), int(parts[4])
                return ("BAR_VALUE", r, c, bar_index, value)
...
class Display:
    ...
    def __init__(self, root: tk.Tk, rows: int = DEFAULT_ROWS, 
        ...
        # Bar widget storage (2 bars per cell in bar rows)
        self.bar_holders: List[List[Optional[tk.Frame]]] = [
            [None] * self.cols_per_row[r] for r in range(self.rows)
        ]
        self.bars: List[List[List[Optional[HorizontalBar]]]] = [
            [[None] * BARS_PER_CELL for _ in range(self.cols_per_row[r])] 
    ...
    def _ensure_bars(self, r: int, c: int) -> None:
        """Ensure bar widgets exist for a cell."""
        ...
        if holder is None:
            ...
            holder.pack(fill="both", expand=True, padx=5, pady=2)
            self.bar_holders[r][c] = holder
            
            # Create two bars stacked vertically
            for bar_idx in range(BARS_PER_CELL):
                bar = HorizontalBar(holder, width=200, height=10)
                bar.pack(fill="both", expand=True, pady=1)
                self.bars[r][c][bar_idx] = bar
    
    def set_bar_value(self, r: int, c: int, bar_index: int, value: int) -> None:
        """Set bar value."""
        if not (0 <= r < self.rows) or not (0 <= c < self.cols_per_row[r]):
            return
        ...
        if not (0 <= bar_index < BARS_PER_CELL):
            logger.warning(f"Invalid bar index {bar_index}, must be 0-{BARS_PER_CELL-1}")
            return
        
        # Ensure bars exist
        self._ensure_bars(r, c)
        
        bar = self.bars[r][c][bar_index]
        if bar:
            bar.set_value(value)
    ...
    def _ensure_ring(self, r: int, c: int, fg_out: str, fg_in: str, 
        ...
        if ring is None:
            ring = DualRing(
                holder, size=size_px, fg_outer=fg_out, fg_inner=fg_in,
...
def main():
    ...
    def drain_and_apply():
        ...
        while True:
            ...
            if kind == "BAR_VALUE":
                _, r, c, bar_index, value = msg
                pending_latest[("BAR", r, c, bar_index)] = value
        ...
        for key, value in list(pending_latest.items()):
            ...
            if key[0] == "BAR":
                _, r, c, bar_index = key
                ui.set_bar_value(r, c, bar_index, value)
                del pending_latest[key]
                applied += 1
