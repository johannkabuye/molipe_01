...
# Grid layout (9 rows instead of 11)
DEFAULT_ROWS = 9
COLS_PER_ROW = [4, 4, 4, 4, 4, 4, 4, 8, 8]

# 0-based indices of rows that use BIG font
BIG_FONT_ROWS = {1, 4, 7}

# Rows that contain bar widgets - DISABLED (no bar rows anymore)
BAR_ROWS = set()  # Empty - no bars
...
# Row heights (pixels) - 9 rows total
ROW_HEIGHTS = [60, 260, 30, 30, 260, 30, 30, 30, 30]
...
class Display:
    ...
    def _ensure_ring(self, r: int, c: int, fg_out: str, fg_in: str, 
        ...
        # Ignore size_px parameter - always use fixed 120px size
        size_px = 120
        ...
        if holder is None:
            holder = tk.Frame(self.cell_frames[r][c], bg="black", bd=0, highlightthickness=0)
            # Fill entire cell instead of fixed size
    ...
    def set_ring_value(self, r: int, c: int, outer: int, inner: int) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Create with defaults - using smaller size 120px instead of 200px
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 120, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
    ...
    def set_ring_text(self, r: int, c: int, text: Optional[str]) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Create ring with defaults - using smaller size 120px
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 120, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
    ...
    def set_ring_extra_arcs(self, r: int, c: int, val1: int, val2: int) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Create ring with defaults if it doesn't exist - using smaller size 120px
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 120, RING_OUTER_ARC_WIDTH, RING_INNER_ARC_WIDTH)
            ring = self.rings[r][c]
