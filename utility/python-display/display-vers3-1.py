...
BIG_FONT_ROWS = {1, 2, 6, 9}  # rows 2, 6, 11
...
class DualRing(tk.Frame):
    ...
    def __init__(self, master, size=200, fg_outer="#606060", fg_inner="#ffffff",
        ...
        self._outer_val = 0
        self._inner_val = 0
        self._center_override = None  # NEW: optional text to show instead of numeric
    ...
    # NEW: allow overriding the center text
    def set_center_text(self, text_or_none):
        self._center_override = (text_or_none if text_or_none not in (None, "") else None)
        self._update_label()
    ...
    def _redraw(self):
        ...
        # center value label (inner value or override)
...
def start_udp_listener(out_queue: Queue):
    ...
    Enqueued messages:
      ...
      ("RING_VALUE", r, c, outer, inner[, text])   # <-- text is optional (NEW)
            ...
            try:
                ...
                if head == "RINGVAL" and len(parts) >= 5:
                    try:
                        ...
                        out_queue.put(("RING_VALUE", r, c, outer, inner, text))
                    except ValueError:
                        pass
                    continue
...
class Display:
    ...
    # NEW: allow setting center text for a ring independently
    def set_ring_text(self, r, c, text_or_none):
        if not (0 <= r < self.rows) or not (0 <= c < self.cols_per_row[r]): return
        ring = self.rings[r][c]
        if ring is None:
            # ensure ring exists with defaults so text has somewhere to go
            self._ensure_ring(r, c, "#606060", "#ffffff", "#000000", 200, 15, 40)
            ring = self.rings[r][c]
        ring.set_center_text(text_or_none)
...
def main():
    ...
    def drain_and_apply():
        while True:
            ...
            if kind == "RING_VALUE":
                # Backwards compatible: may be ("RING_VALUE", r, c, outer, inner) OR with extra text
                if len(msg) >= 6:
