...
class DualRing(tk.Frame):
    ...
    def _redraw(self) -> None:
        ...
        # Calculate padding for extra arcs with even spacing
        gap = RING_EXTRA_ARC_GAP
        extra_arc_width = RING_EXTRA_ARC_WIDTH
        extra_arc1_pad = outer_pad - gap - extra_arc_width // 2
        ...
        # Extra arc 1 (second from outside)
    ...
    def _update_extents(self) -> None:
        ...
        # Calculate radii for extra arcs
        outer_pad = max(self._w_outer // 2 + 2, 4)
        gap = RING_EXTRA_ARC_GAP
        extra_arc_width = RING_EXTRA_ARC_WIDTH
        extra_arc1_pad = outer_pad - gap - extra_arc_width // 2
        ...
        # Angles in canvas are: 0° = 3 o'clock, 90° = 12 o'clock, etc.
        # Our start is 210° (7 o'clock), extent is negative (clockwise)
        peak_angle1_deg = RING_START_ANGLE + ext_extra1  # in canvas degrees
        peak_angle2_deg = RING_START_ANGLE + ext_extra2
        
        # Convert to radians for calculation (canvas uses degrees but math uses radians)
