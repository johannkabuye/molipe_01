...
class DualRing(tk.Frame):
    ...
    def __init__(self, master, size: int = 200, fg_outer: str = "#606060", 
        ...
        self._display_size = int(size)  # Canvas display size
        self._bg = bg
    ...
    def _redraw(self) -> None:
        ...
        # Extra arc 2 (lighter colored, outermost)
...
class Display:
    ...
    def _build_ui(self) -> None:
        ...
        for r in range(self.rows):
            ...
            for c in range(cols):
                ...
                # Set anchor based on row (rows 3 and 7 align to top-center)
                if r in {2, 6}:  # Row 3 (index 2) and Row 7 (index 6)
                    anchor = "n"  # North = top-center
                else:
                    anchor = "w"  # West = left (vertically centered)
    ...
    def _ensure_ring(self, r: int, c: int, fg_out: str, fg_in: str, 
        ...
        # Canvas will fill the cell automatically
        # size_px parameter is ignored
        ...
        if holder is None:
            holder = tk.Frame(self.cell_frames[r][c], bg="black", bd=0, highlightthickness=0)
            # Position canvas at top-center, let it overflow downward
            holder.place(relx=0.5, rely=0.0, anchor="n")
            self.ring_holders[r][c] = holder
        
        ring = self.rings[r][c]
        if ring is None:
            # Ring canvas will auto-size to cell, arcs drawn at 180px
            ring = DualRing(
    ...
    def set_ring_value(self, r: int, c: int, outer: int, inner: int) -> None:
        ...
        ring = self.rings[r][c]
        if ring is None:
            # Canvas 280px, arcs drawn at 175px
